...
public class LinkListener extends ListenerAdapter {
    ...
    private void giveRoles(Member member) {
        if (config.getLongList("discord.roles").isEmpty()) return;
        Guild guild = jda.getGuildById(config.getLong("guild"));
        Member bot = guild.getSelfMember();
        ...
        for (long l : config.getLongList("discord.roles")) {
            Role role = guild.getRoleById(l);
            if (botHighestRole != null && role != null &&
                    botHighestRole.getPosition() > role.getPosition()) {
